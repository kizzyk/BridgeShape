...
def visualize_pointcloud_batch_as_gif(
        path,
        pointclouds,
        elev=30,
        azim=225,
        interval=500,  # 设置帧之间的间隔，单位为毫秒
        save_as_gif=True,  # 控制是否保存为GIF
        fig_size=(10, 10),  # 更小的尺寸
):
    ...
    def update_plot(frame_idx):
        plt.clf()  # 清除前一帧的绘制
        idx = batch_size - 1 - frame_idx  # 反向索引
        pc = pointclouds[idx]
    ...
    # 创建动图
    ani = FuncAnimation(fig, update_plot, frames=batch_size, interval=interval)
