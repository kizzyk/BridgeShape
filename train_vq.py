...
from models.models_vq.base_model import create_model

from utils.utils_vq.distributed import (
    get_rank,
    synchronize,
    reduce_loss_dict,
    reduce_sum,
...
import torch
from utils.utils_vq.visualizer import Visualizer
#os.environ["CUDA_VISIBLE_DEVICES"] = "6"

def train_main_worker(opt, model, train_dl, test_dl, visualizer):
    ...
    for iter_i in range(opt.total_iters):
        ...
        if get_rank() == 0:
            ...
            if iter_i % opt.display_freq == 0 and iter_i!=0:
...
if __name__ == "__main__":
    ...
    train_dl, test_dl ,_= CreateDataLoader(opt)
    train_ds, test_ds = train_dl.dataset, test_dl.dataset

    dataset_size = len(train_ds)
    if opt.dataset_mode == 'shapenet_lang' or opt.dataset_mode == 'ControlledEPNDataset':
    ...
    train_main_worker(opt, model, train_dl, test_dl, visualizer)
